...
// Connect to MongoDB mongodb://localhost/doctor_app
// mongoose
//   .connect("mongodb://localhost/doctor_app", {
...
app.put("/api/appointments/:id/visited", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to mark the appointment as visited" });
  }
});
...
app.put("/api/appointments/:id/notvisited", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to mark the appointment as not visited" });
  }
});
...
app.post("/api/appointments", async (req, res) => {
  ...
  try {
    ...
    const existingAppointment = await Appointment.findOne({
      ...
      date,
      time,
    }); // Check for existing appointment with date and time
    ...
    const appointment = new Appointment({
      ...
      date,
      time,
    }); // Include time in the appointment
...
app.get("/appointments", async (req, res) => {
  ...
  try {
    ...
    const filteredAppointments = appointments.filter(
      (appointment) => appointment.doctor._id.toString() === doctorId
    );
